...
const ChatDetails = ({ chatId }) => {
  ...
  const sendPhoto = async (result) => {
    try {
      ...
      if (!res.ok) {
        throw new Error(`Failed to send photo: ${res.statusText}`);
      }

      console.log("Photo sent successfully:", result.info.secure_url);
    } catch (error) {
  ...
  // console.log("othermemeber : ", otherMembers[0]);
            ...
            <CldUploadButton
              ...
              onSuccess={(result) => {
                if (result?.info?.secure_url) {
                  ...
                  sendPhoto(result); // Pass the result to sendPhoto
                } else {
                  console.error(
                    "Upload failed or no secure_url in result:",
                    result
                  );
                }
              }}
